...
void setup()
{
  ...
  Serial.print(rtcNowDebug.year());
  Serial.print("-");
  Serial.print(rtcNowDebug.month());
  Serial.print("-");
  Serial.print(rtcNowDebug.day());
  Serial.print(" ");
  Serial.print(rtcNowDebug.hour());
  Serial.print(":");
  Serial.print(rtcNowDebug.minute());
  Serial.print(":");
  ...
  while (ntpEpoch < validThreshold && (millis() - startNtp) < 3000)
  { // 0.5s max wait
    delay(100);
...
void loop()
{
  ...
  if (hour >= BFL && hour < BFH)
    newMeal = BREAKFAST;
  else if (hour >= LFL && hour <= LFH)
    newMeal = LUNCH;
  else if (hour >= DFL && hour <= DFH)
    newMeal = DINNER;
  else
    newMeal = NONE;
  currentMeal = newMeal;
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    if (rssi >= -55)
      g_wifiLevelIndex = 4; // Excellent
    else if (rssi >= -65)
      g_wifiLevelIndex = 3; // Good
    else if (rssi >= -75)
      g_wifiLevelIndex = 2; // Fair
    else if (rssi >= -85)
      g_wifiLevelIndex = 1; // Weak
    else
      g_wifiLevelIndex = 0; // No/very poor signal
  }
  else
  {
  ...
  if (millis() - lastBattRead > 500)
  {
    ...
    float vBatSum = 0.0f;
    for (int i = 0; i < vBatCount; ++i)
      vBatSum += vBatReadings[i];
  ...
  if (hour >= BFL && hour < BFH)
    token_data.meal = BREAKFAST;
  else if (hour >= LFL && hour <= LFH)
    token_data.meal = LUNCH;
  else if (hour >= DFL && hour <= DFH)
    token_data.meal = DINNER;
  else
    token_data.meal = NONE;
  ...
  if (token_data.meal != NONE)
  {
    ...
    if (((now - lastSensorSend) >= TIME_SYNC_DATA_INTERVAL_MS) || ((g_tokenCount != g_tokenCountPrevious) && (now - lastSensorSend) >= 1000))
    { // 10,000 ms = 10 s
      lastSensorSend = now;
...
void drawScreen()
{
  ...
  if (g_bundleAdd > 0)
  {
    ...
    if (blinkOn)
    {
      ...
      if (bundleX < 0)
        bundleX = 0; // safety: never negative
...
void fetchPeerDataIfNeeded()
{
  ...
  {
    if (nowFetch - lastPeerFetch > 10000)
    { // 10s interval
